 ...
 * 	 이때, 조회된 행들의 집합을 'RESULT SET" (조회결과의 집합)이라고 함 
 * 
...
-- EMAIL에서 _ 앞 글자가 세글자인 사원 이름, 이메일 조회 
SELECT EMP_NAME, EMAIL
FROM EMPLOYEE 
WHERE EMAIL LIKE '____%';
-- 문제점 : _를 기준점으로 삼았으나, 패턴 중 '_'와
-- 동일한 표기법으로 작성되어 구분되지 않음 

-- 해결 방법 : LIKE의 ESCAPE OPTION을 이용해 _를 구분 
...
-- 연산자 우선순위 
		...
		/*
		 * 기본 키(primary key) : 중복되지 않고 행을 구분하는 키 
		   ->각 행 구분 가능 
			외래키 (Foreign Key) : 외부에 있는 기본키/참조키 
	...
	----------------------------------------------------------------------------
		 ...
		 * 1. FROM절 
		 * 2. WHERE절 
		 * 4. ORDER BY 컬럼명 | 별칭 | 컬럼 순서 [ASC | DESC] [NULLS FIRST | LAST] //[] 없이 작성      
		 * 							      	(오름차순:기본값) | (내림차순) 
		 * 
		 * */
