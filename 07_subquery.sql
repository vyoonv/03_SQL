...
-- 부서가 기술지원부인 모든 컬럼 
SELECT * FROM (SELECT EMP_NAME 이름, DEPT_TITLE 부서 FROM EMPLOYEE 
...
FROM EMPLOYEE LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) GROUP BY DEPT_CODE, DEPT_TITLE
ORDER BY 평균급여 DESC)
WHERE ROWNUM <= 3; 
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, HIRE_DATE
FROM EMPLOYEE WHERE (DEPT_CODE, JOB_CODE) =
...
SELECT EMP_ID, EMP_NAME, MANAGER_ID, EMP_NO, HIRE_DATE FROM EMPLOYEE 
