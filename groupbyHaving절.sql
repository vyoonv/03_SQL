 ...
 * ORDER BY 컬럼명 | 별칭 | 컬럼순번 정렬방식 (ASC/DESC) [NULLS FIRST |LAST] 
 * 
 * */
 ...
 * - 여러개의 결과 값을 산출하기 위해 그룹함수가 적용된 그룹의 기준을 ORDER BY절에 기술하여 사용  
 * */
...
FROM EMPLOYEE 
GROUP BY DECODE( SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭사용X (SELECT절보다 해석 순서가 앞)
ORDER BY "인원 수";
...
-- EMPLOYEE 테이블에서 부서코드가 직급별 2000년도 이후 (2000년 포함) 입사자들의 
-- 급여 합을 조회 (직급코드 오름차순)
...
GROUP BY JOB_CODE
ORDER BY 1; --1:JOB_CODE(SELECT 첫번째순서) 
...
-- DEPT코드로 그룹을 나누고, 나눠진 그룹 내에서 JOB_CODE로 또 그룹을 분류 --> 세분화
...
FROM EMPLOYEE 
-- WHERE SALARY >= 3000000 --> 한 사람의 급여가 3백만 이상이라는 조건 
GROUP BY DEPT_CODE 
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE 그룹 중 급여 평균이 300백만 이상인 그룹만 조회 
ORDER BY DEPT_CODE ; 
...
--ROLLUP : GROUP BY 절에서 가장 먼저 작성된 칼럼의 중간 집계를 처리하는 함수 
SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE 
